...
def callback(actuator_index):
    ...
    if csv_Data is not None:
        ...
        match_Row = csv_Data.loc[abs(csv_Data.iloc[:, actuator_index+1] - error) <= 0.03]
        

        if not match_Row.empty:
            closest_duty_cycle = csv_Data.iloc[closest_row_index, 0]  # Get the corresponding duty cycle (column 0)
        else:
            #rospy.logwarn(f"No match found for rounded correction: {error} within tolerance of 0.03")
            closest_duty_cycle = rospy.get_param(f"/desired_duty_cycle_actuator_1_row_91")
            print("////")
            print(closest_duty_cycle)
            print("////")
    ...
    duty_cycle_pub[actuator_index - 1].publish(closest_duty_cycle)
#   rospy.loginfo(f"Actuator {actuator_index} duty cycle adjusted: {closest_duty_cycle}")
